...
public class SimpleDFA2 extends DefaultLearningExample.DefaultDFALearningExample<Symbol<Character>> {
    ...
    public static <S, T, A extends MutableDFA<S, ? super Symbol<Character>>> A constructMachine(A machine) {
        return AutomatonBuilders.forDFA(machine).withInitial("s0").from("s0").on(IN_ZERO).to("s1").from("s1")
                .on(IN_ZERO).to("s2").from("s2").on(IN_ZERO).to("s3").from("s3").on(IN_ZERO).to("s4").from("s4")
                .on(IN_ZERO).to("s5").from("s5").on(IN_ZERO).to("s0").withAccepting("s0").create();
    }
